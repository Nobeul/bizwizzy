$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price_inc_tax', function() {
        ...
        var price_including_tax_in_pos = (line_total/quantity); // new
        __write_number(tr.find('input.pos_unit_price_inc_tax'), price_including_tax_in_pos); // new

        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
        var error_msg_td = $(this).closest('tr').find('.pos_unit_price_inc_tax').closest('td');
        if (__read_number($(this)) < parseFloat($(this).data('min-unit-price'))) {
            error_msg_td.find('label.error').remove();
            error_msg_td.append( '<label class="error "> Amount should be more than or equal to ' + parseFloat($(this).data('min-unit-price') + '</label>'));
            disable_pos_form_actions();
        } else {
            error_msg_td.find('label.error').remove();
            enable_pos_form_actions();
...
function pos_each_row(row_obj) {
    ...
    // var total_price = __read_number(row_obj.find('input.pos_line_total'));
    // __write_number(row_obj.find('input.pos_unit_price_inc_tax'), unit_price_inc_tax);
    // __write_number(row_obj.find('input.pos_unit_price_inc_tax'), total_price);
